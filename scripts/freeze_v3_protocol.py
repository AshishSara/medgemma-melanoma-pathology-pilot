#!/usr/bin/env python3
"""Freeze the v3 formal protocol after a passing, provenance-valid development run."""
...
PROTOCOL_IDENTIFIER = "fresh-heldout-pilot-v3"
...
def load_and_validate_manifest(root: Path) -> List[Dict[str, str]]:
    manifest_path = root / "data" / "v3" / "report_manifest.csv"
    try:
        with manifest_path.open(newline="", encoding="utf-8") as handle:
            rows = list(csv.DictReader(handle))
    except OSError as exc:
        raise ProtocolFreezeError(f"Cannot read v3 manifest: {manifest_path}") from exc
...
def validate_manifest_rows(rows: Sequence[Mapping[str, str]]) -> List[Dict[str, str]]:
    ...
    if len(rows) != EXPECTED_MANIFEST_OUTPUT_COUNT:
        _fail(f"Expected {EXPECTED_MANIFEST_OUTPUT_COUNT} v3 manifest rows, found {len(rows)}")
    if any(None in row for row in rows):
        _fail("V3 manifest has malformed or extra columns")
    if any(row.get("dataset_version") != "pilot-v3" for row in rows):
        _fail("V3 manifest contains a non-v3 dataset row")

    keys = [(row.get("document_id"), row.get("condition")) for row in rows]
    if len(set(keys)) != len(keys):
        _fail("V3 manifest contains duplicate document/condition keys")
    ...
    if observed_matrix != expected_matrix:
        _fail("V3 manifest does not match the frozen case/template/condition matrix")
    for row in rows:
        document_id = row["document_id"]
        condition = row["condition"]
        expected_paths = {
            "source_text_path": f"data/v3/source_text/{document_id}.txt",
            "ground_truth_path": f"data/v3/ground_truth/{document_id}.json",
            "pdf_path": f"output/v3/pdf/{condition}/{document_id}.pdf",
            "image_path": f"output/v3/rendered/{condition}/{document_id}.png",
        }
...
def collect_corpus_artifacts(
    ...
    corpus_roots = (
        root / "data" / "v3" / "source_text",
        root / "data" / "v3" / "ground_truth",
        root / "output" / "v3" / "pdf",
        root / "output" / "v3" / "rendered",
    )
...
def partition_corpus_artifacts(
    ...
    operational_paths = {"data/v3/report_manifest.csv"}
    evaluation_only_paths = {"data/v3/cases.csv"}
    for row in rows:
...
def _development_operational_ast(source: bytes) -> str:
    try:
        tree = ast.parse(source.decode("utf-8"))
    except (SyntaxError, UnicodeDecodeError) as exc:
        raise ProtocolFreezeError("Could not parse development inference source") from exc
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.Assign) and any(
            isinstance(target, ast.Name) and target.id == "REQUIRED_LOCKED_SOURCE_PATHS"
...
def validate_post_development_source_hardening(
    ...
    """Permit only formal lock-validation hardening after development inference."""

    inference_relative = "scripts/run_v3_inference.py"
    development_inference = _committed_blob(root, development_commit, inference_relative)
    current_inference = (root / inference_relative).read_bytes()
    if _development_operational_ast(development_inference) != _development_operational_ast(
        current_inference
    ):
        _fail(
            "Inference semantics outside REQUIRED_LOCKED_SOURCE_PATHS and "
            "verify_protocol_lock changed after development execution"
        )

    pilot_utils_relative = "scripts/pilot_utils.py"
    development_pilot_utils = _committed_blob(
        root,
        development_commit,
        pilot_utils_relative,
    )
    current_pilot_utils = (root / pilot_utils_relative).read_bytes()
    if development_pilot_utils != current_pilot_utils:
        _fail("scripts/pilot_utils.py changed after development execution")

    return {
        "classification": "formal_provenance_hardening_only",
        "development_inference_script_sha256": sha256_bytes(development_inference),
        "formal_inference_script_sha256": sha256_bytes(current_inference),
        "ignored_ast_nodes_for_equivalence_check": [
            "REQUIRED_LOCKED_SOURCE_PATHS assignment",
            "verify_protocol_lock function",
        ],
        "pilot_utils_unchanged_since_development": True,
    }
...
def _expected_run_record_values(
    ...
    return {
        ...
        "manifest_sha256": sha256_file(root / "data" / "v3" / "report_manifest.csv"),
        ...
        "inference_script_path": "scripts/run_v3_inference.py",
...
def common_runtime(records: Sequence[Mapping[str, Any]]) -> Dict[str, str]:
...
def _evidence_gate_summary(compiler_logs: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    from evaluate_v3 import _accepted_decision_has_valid_evidence
...
def require_development_gate(
    ...
    if overall.get("output_count") != DEVELOPMENT_OUTPUT_COUNT:
        _fail("Development overall output count is not 12")
    if overall.get("schema_valid_count") != DEVELOPMENT_OUTPUT_COUNT:
        _fail("Development canonical schema count is not 12/12")
    for condition in CONDITIONS:
        if by_condition[condition].get("output_count") != EXPECTED_DEVELOPMENT_CONDITION_COUNT:
            _fail(f"Development condition {condition} does not contain six outputs")

    criteria = {
        "provenance_12_of_12": True,
        "schema_100_percent": overall["schema_valid_rate"] == 1.0,
        "evidence_acceptance_100_percent": (evidence.get("accepted_non_null_evidence_rate") == 1.0),
        "pooled_exact_at_least_90_percent": (
            overall["field_exact_match"] >= PASS_CRITERIA["pooled_field_exact_match_minimum"]
        ),
        "each_condition_exact_at_least_85_percent": all(
            by_condition[condition]["field_exact_match"]
            >= PASS_CRITERIA["each_condition_field_exact_match_minimum"]
            for condition in CONDITIONS
        ),
        "unsupported_at_most_2_percent": (
            overall["unsupported_field_rate"] <= PASS_CRITERIA["unsupported_field_rate_maximum"]
        ),
...
def validate_complete_development_iteration(
    ...
    source_hardening = validate_post_development_source_hardening(
        root,
        repository_commit,
    )
    ...
    for key in ("change_note", "selection_rule"):
    ...
    gate = require_development_gate(recomputed["final_pipeline"], evidence)
    return {
        "runtime": runtime,
...
def collect_documented_development_iterations(
    ...
    development_root = root / "results" / "v3" / "development"
    ...
    for path in development_root.glob("iteration-*"):
        ...
        if iteration not in {1, 2}:
            _fail("More than two development iterations are documented")
        discovered_iterations[iteration] = path
    expected_iterations = set(range(1, selected_iteration + 1))
...
def freeze_protocol(
    ...
    *,
    ...
    if selected_iteration not in {1, 2}:
        _fail("Selected development iteration must be 1 or 2")
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"V3 protocol freeze refused: {exc}") from exc
    print(
