#!/usr/bin/env python3
"""Evaluate the frozen 20-input formal v3 pilot without test-set peeking."""
...
METRICS_PATH = ROOT / "results" / "v3" / "formal" / "metrics.json"
OUTPUT_SEAL_PATH = ROOT / "results" / "v3" / "formal" / "output_seal.json"
...
def _load_formal_manifest() -> List[Dict[str, str]]:
    ...
    if len(rows) != FORMAL_OUTPUT_COUNT:
        raise ValueError(
            f"Expected {FORMAL_OUTPUT_COUNT} formal v3 manifest rows, found {len(rows)}"
        )
    keys = {(row["document_id"], row["condition"]) for row in rows}
    if len(keys) != FORMAL_OUTPUT_COUNT:
        raise ValueError("Formal v3 manifest has duplicate document/condition keys")
    if {row["condition"] for row in rows} != set(CONDITIONS):
        raise ValueError("Formal v3 manifest does not contain both frozen conditions")
    return rows
...
def _artifact_descriptor_errors(
    ...
    if not path.is_file():
        return [f"{name}: missing {_relative(path)}"]
...
def _expected_record_values(
    ...
    return {
        ...
        "manifest_path": _relative(V3_MANIFEST),
...
def _preflight_row(
    ...
    for path in paths.values():
        if not path.is_file():
            issues.append(f"missing {_relative(path)}")
    if issues:
...
def _validate_execution_attempt_ledger(
    ...
    for attempt_id, event_map in sorted(attempts.items()):
        ...
        if terminal_name == "completed":
            ...
            if completed_new + verified_existing != FORMAL_OUTPUT_COUNT:
                issues.append(
                    f"completed execution attempt {attempt_id} does not account for all formal rows"
                )
        elif not isinstance(terminal.get("resume_permitted_for_current_row"), bool):
            issues.append(f"failed execution attempt {attempt_id} has no valid resume decision")
...
def _verify_evaluation_only_corpus(
    ...
    expected_paths = {"data/v3/cases.csv"}
    for row in all_rows:
    ...
    for relative_path in sorted(expected_paths):
        ...
        if (
            ...
            or (
                ...
                and not relative_path.startswith(("data/v3/source_text/", "data/v3/ground_truth/"))
            )
...
def evaluate() -> Dict[str, Any]:
    ...
    if any(item["issues"] for item in preflight):
        ...
        print(
            f"V3 evaluation stopped before ground truth: "
    ...
    write_json(METRICS_PATH, payload)
    print(
        f"V3 evaluation {payload['status']}: "
